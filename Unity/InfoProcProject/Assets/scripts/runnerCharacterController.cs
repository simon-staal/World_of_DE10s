...
public class runnerCharacterController : MonoBehaviour
{
    ...
    [SerializeField] Score scorecount;
    public string x;
    bool setbomb = false;
    float previous_ver;
    float previous_hor;
    ...
    void Update()
    {
        ...
        if (fire == true)
        {
            if (!bombed)
            {
                ...
                {
                    Instantiate(fire_left, transform.position + (Vector3.left * 0.4f) + Vector3.forward, transform.rotation);
                    Instantiate(bullet, transform.position + (Vector3.left * 0.4f) + Vector3.forward, Quaternion.Euler(0f, 0f, 180f));
                }
                else if (animator.GetCurrentAnimatorStateInfo(0).IsName("redright"))
                {
                    Instantiate(fire_right, transform.position + (Vector3.right * 0.4f) + Vector3.forward, transform.rotation);
                    Instantiate(bullet, transform.position + (Vector3.right * 0.4f) + Vector3.forward, Quaternion.Euler(0f, 0f, 0f));
                }
                else if (animator.GetCurrentAnimatorStateInfo(0).IsName("reddown"))
                {
                    Instantiate(fire_down, transform.position + (Vector3.down * 0.4f) + Vector3.forward, transform.rotation);
                    Instantiate(bullet, transform.position + (Vector3.down * 0.4f) + Vector3.forward, Quaternion.Euler(0f, 0f, 270f));
                }
                else if (animator.GetCurrentAnimatorStateInfo(0).IsName("redup"))
                {
                    Instantiate(fire_up, transform.position + (Vector3.up * 0.4f) + Vector3.forward, transform.rotation);
                    Instantiate(bullet, transform.position + (Vector3.up * 0.4f) + Vector3.forward, Quaternion.Euler(0f, 0f, 90f));
                }
